...
const ANIM_LINES = [
  '[  OK  ] Initializing emtypyie-core v2.5.6 (Wandering Witches)',
  ...
  '[  OK  ] Migration v2.5.x -> v2.5.6: 0 steps required',
  ...
  '[  OK  ] Emtypyie CLI v2.5.6 ready -- type /help for commands',
...
const ANIM_INTERVAL_MS = ANIM_TOTAL_MS / ANIM_TARGET_COUNT;
...
async function playStartupAnimation(options = {}) {
  ...
  for (let i = 0; i < ANIM_TARGET_COUNT; i++) {
    ...
    if (ANIM_INTERVAL_MS > 0) await sleep(ANIM_INTERVAL_MS);
  }
}
...
async function interactive() {
  ...
  console.log(BANNER);
  console.log(t.retro('  EMTYPYIE CLI v2.5.6'));
  console.log(t.retroDim('  "Wandering Witches"'));
  console.log();
  console.log(t.retroDim(`
  ─────────────────────────────────────────────
