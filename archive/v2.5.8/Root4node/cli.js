...
const ANIM_LINES = [
  '[  OK  ] Initializing emtypyie-core v2.5.8 (Wandering Witches)',
  ...
  '[  OK  ] Emtypyie CLI v2.5.8 ready -- type /help for commands',
  '[  OK  ] Welcome back. The witches are wandering.',
  '[  OK  ] ████████████████████████████████████████ 100%',
];

const ANIM_TARGET_COUNT = ANIM_LINES.length;
...
async function playStartupAnimation(options = {}) {
  ...
  for (let i = 0; i < ANIM_TARGET_COUNT; i++) {
    ...
    let delay = ANIM_LINE_DELAY_MS;
    if (i === Math.floor(ANIM_LINES.length / 2)) delay = ANIM_LINE_DELAY_MS * 4;
    if (delay > 0) await sleep(delay);
  }
}

const ANIM_LINE_DELAY_MS = 110;
...
async function interactive() {
  ...
  console.log(BANNER);
  console.log(t.retro('  EMTYPYIE CLI v2.5.8'));
  console.log(t.retroDim('  "Wandering Witches"'));
  console.log();
  console.log(t.retroDim(`
  ─────────────────────────────────────────────
