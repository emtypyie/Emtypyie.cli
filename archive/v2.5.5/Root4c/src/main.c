...
static const int SEG_BOUNDS[6] = { 40, 90, 130, 180, 240, 300 };
static const int SEG_INTERVAL[6] = { 6, 14, 3, 7, 14, 2 };

/* Pauses: a long one (download/load) and a small beat */
#define PAUSE_MAIN_LINE 100
#define PAUSE_MAIN_MS   500
#define PAUSE_SMALL_LINE 150
#define PAUSE_SMALL_MS  150
...
static void startup_animation_play(void) {
    ...
    for (int i = 0; i < ANIM_TARGET_COUNT; i++) {
        ...
        int interval_ms = SEG_INTERVAL[seg];
        int freq = 760 + seg * 50;
        ...
        if (i == PAUSE_MAIN_LINE) {
            printf("  %s\r", "[ ... ] Downloading manifests...");
            fflush(stdout);
            Beep(520, 180);
            Sleep(PAUSE_MAIN_MS);
            printf("  %s\n", "[  OK  ] Downloaded manifests (12.4MB)");
        } else if (i == PAUSE_SMALL_LINE) {
            printf("  %s\r", "[ ... ] ");
            fflush(stdout);
            Beep(620, 90);
            Sleep(PAUSE_SMALL_MS);
            printf("  %s\n", "[  OK  ] ");
        }
    }
}
