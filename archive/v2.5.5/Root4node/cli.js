...
const ANIM_LINES = [
  '[  OK  ] Initializing emtypyie-core v2.5.5 (Wandering Witches)',
  ...
  '[  OK  ] Migration v2.5.x -> v2.5.5: 0 steps required',
  ...
  '[  OK  ] Emtypyie CLI v2.5.5 ready -- type /help for commands',
...
/* Asymmetric speed profile: mid -> slow -> fast -> mid -> slow -> fast */
const SEG_BOUNDS = [40, 90, 130, 180, 240, 300];
const SEG_INTERVAL = [6, 14, 3, 7, 14, 2];

/* Pauses: a long one (download/load) and a small beat */
...
async function playStartupAnimation(options = {}) {
  ...
  for (let i = 0; i < ANIM_TARGET_COUNT; i++) {
    ...
    process.stdout.write('\x07'); // eDex-UI-style per-line bell
...
async function interactive() {
  ...
  console.log(BANNER);
  console.log(t.retro('  EMTYPYIE CLI v2.5.5'));
  console.log(t.retroDim('  "Wandering Witches"'));
  console.log();
  console.log(t.retroDim(`
  ─────────────────────────────────────────────
