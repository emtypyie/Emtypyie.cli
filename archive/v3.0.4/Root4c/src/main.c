...
int main(int argc, char *argv[]) {
    ...
    static char saved_theme[512] = {0};
    char *emty_dir = get_emty_dir();

    char theme_path[1024];
    snprintf(theme_path, sizeof(theme_path), "%s%ctheme.txt", emty_dir, PATH_SEP);
    if (file_exists(theme_path)) {
        char *loaded = read_file(theme_path);
        if (loaded) {
            char *nl = strchr(loaded, '\n');
            if (nl) *nl = '\0';
            strncpy(saved_theme, loaded, sizeof(saved_theme) - 1);
            free(loaded);
    ...
    for (int i = 1; i < argc; i++) {
        ...
        else if (strcmp(argv[i], "--upgrade") == 0) do_upgrade = 1;
    }
    ...
    if (show_version) {
        if (g_json) {
            json_emit_str("version", 1, "EMTYPYIE CLI v3.0.4 \"Baking Bread\"");
        } else {
    ...
    if      (strcmp(cmd, "/help") == 0) {
        ...
        if (g_json) {
            json_emit_str(cmd, 1, "EMTYPYIE CLI v3.0.4 \"Baking Bread\"");
        } else {
            printf("%s", retro(BANNER));
            printf("  %s\n", retro_accent("EMTYPYIE CLI v3.0.4"));
            printf("  %s\n", retro_dim("\"Baking Bread\""));
